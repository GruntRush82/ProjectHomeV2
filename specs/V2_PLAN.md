...
> **Status:** VERSION 1.0 COMPLETE — All 6 phases done. 312 tests passing.
>
> **Branch:** `v2` (created from `master`)
>
> **Last updated:** 2026-02-23 (v1.0 complete — awaiting new requirements for next version)
...
- **Phase 5 COMPLETE** — Achievements, XP & Gamification: 2 new models (Achievement catalog, UserAchievement per-user unlocks), 2 new services (XP grant/level-up, achievement checking engine with trigger-based unlock), 14 seeded achievements across 3 categories (chores/streaks/bank) with bronze/silver/gold tiers, 10-level XP system (Rookie→Ultimate), integration hooks in chores (XP on completion: 10 static/25 rotating, weekly bonus doubling, streak/fire_mode tracking), bank (cashout/deposit/interest/goal achievements), missions (500 XP on completion), achievements blueprint (8 routes: notifications, catalog, profile page, icon/theme APIs), profile.html (stats, icon picker with 20 starter + 8 level-unlocked + 2 mission-earned icons, theme colour picker with progressive unlock), tiered notification UI (bronze toast 3s, silver banner 5s, gold fullscreen 8s), Fire Mode (+50% allowance at 3+ week streak), progressive CSS-only level effects on login cards and app-wide (L4-10), Fire Mode visuals (fire border on login cards, bank banner, nav accent). 291 tests passing

**What's next:**
- New requirements incoming for next version
...
| 2026-02-17 | **Phase 5 COMPLETE — Achievements, XP & Gamification.** 5A: Achievement + UserAchievement models, User model additions (perfect_weeks_total, fire_mode), seed script for 14 achievements. 5B: XP service (10-level system Rookie→Ultimate, grant_xp, get_level_info), achievement service (trigger-based engine: chore_complete, weekly_reset, savings_deposit, cashout, interest_credited, savings_goal_completed). 5C: Integration hooks — chores blueprint (XP on completion: 10 static/25 rotating, no undo deduction, weekly bonus XP doubling, streak tracking with perfect_weeks_total, Fire Mode activation at 3+ weeks), bank blueprint (cashout/deposit achievements, savings goal completion check), missions blueprint (500 XP on mission complete), Fire Mode 50% allowance boost. 5D: Achievements blueprint (8 routes: notifications gold-first, catalog, dismiss, profile page + icon/theme APIs), tiered notification UI in base.html (bronze toast 3s, silver banner 5s, gold fullscreen 8s), level-up overlay, Fire Mode activation overlay. 5E: profile.html (Alpine.js — stats, XP progress bar, achievement showcase + full catalog, icon picker with 20 starter + 8 level-unlocked + 2 mission-earned icons, theme colour picker with progressive unlock at L1/3/5/7). 5F: Progressive CSS-only level effects on login cards (L4-5 glow, L6-7 gradient border, L8-9 rotating + sparkle, L10 rainbow + crown), app-wide effects (ambient gradient, button glow, nav accent, scan line, heading shimmer), Fire Mode visuals (fire border on login cards, bank banner, nav accent), data-level/data-fire-mode attributes on body + login cards. 5G: 10 E2E tests (full week cycle, XP accumulation, streak progression, bank chain, multi-achievement, level-up from achievement XP, Fire Mode full cycle), docs updated. **291 tests all passing.** | Phase 5 done. Begin Phase 6 if planned. |
| 2026-02-23 | **Phase 6 COMPLETE — Admin Dashboard. VERSION 1.0 COMPLETE.** 6A: Admin blueprint (12 routes: overview with bank data, user CRUD, 7-key AppConfig management, PIN change, trusted IP list/revoke, digest trigger). 6B: Weekly digest email service (`app/services/digest.py`) with per-kid chore/allowance/bank/level blocks, dry-runs when no Mailgun keys. 6C: APScheduler `weekly_reset` cron job registered in `__init__.py` (Mon 00:01). 6D: `is_admin` added to `/api/profile` response. 6E: `_weekly_archive()` wired to digest. 6F: `admin.html` single-page tabbed UI (Overview, Users, Settings, Trusted IPs) with Alpine.js, icon emoji fix. 6G: Profile page Admin Panel button (orange, admin-only). 6H: Visual tests for admin panel (15 checks including icon rendering, tab navigation, accordion, settings pre-fill, non-admin redirect). **312 tests all passing.** | v1.0 complete. Awaiting new requirements. |
